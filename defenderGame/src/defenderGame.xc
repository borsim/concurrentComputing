...
void userAnt(chanend fromButtons, chanend toVisualiser, chanend toController) {
  ...
  while (1) {
    ...
    if (buttonInput == 13) {
        attemptedAntPosition = (userAntPosition + 21) % 22;
    } else if (buttonInput == 14) {
        attemptedAntPosition = (userAntPosition + 1) % 22;
    }
...
void attackerAnt(chanend toVisualiser, chanend toController) {
  ...
  while (running) {
      attemptedAntPosition = (attackerAntPosition + currentDirection) % 22;
      toController <: attemptedAntPosition;
      toController :> moveForbidden;
      if (moveForbidden == 0 && moveCounter % 31 != 0 && moveCounter % 37 != 0) {
          attackerAntPosition = attemptedAntPosition;
      }
      else {
          if (currentDirection == 1) currentDirection = 21;
